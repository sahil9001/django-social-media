...
def profile(request):
    ...
    return render(request, "users/profile.html",{"posts": post_list,"friends":friend_list})
...
def friendrequest(request):
    ...
    return render(request, "users/friendrequests.html",{"friendrequests":friendrequest_list})
...
def otherprofile(request,id):
    ...
    if user == request.user:
        is_friend = True
    return render(request, "users/otherprofile.html",{"user":user,"is_friend": is_friend})
...
def send_friend_request(request):
    if request.method == "POST":
        ...
        try:
            ...
            receiver = None
        if receiver:
            friendrequest, flag = FriendRequest.objects.get_or_create(sender=request.user, receiver=receiver)
            friendrequest.save()
...
def accept_friend_request(request):
    if request.method == "POST":
        ...
        print(user.fullname)
        try:
            friendrequest = FriendRequest.objects.get(sender=user, receiver=request.user)
...
def decline_friend_request(request):
    if request.method == "POST":
        try:
            ...
            user = None
        try:
            friendrequest = FriendRequest.objects.get(sender=request.user, receiver=user)
        except FriendRequest.DoesNotExist:
            friendrequest = None
        if friendrequest:
...
def friend_list(request):
    ...
    if friendlist:
        ...
        return render(request, "users/friends.html",{"friends":friends,"mutual_friends" : mutual_friends})
    else:
